...
def main(argv: list[str] | None = None) -> int:
    ...
    for item in items:
        ...
        if result.success and result.target_path:
            record = pdf_record(result.target_path, source_urls=result.attempts)
            manifest = update_manifest(out_dir, record)
            print(f"SAVED publisher={plan.publisher} path={result.target_path} sha256={record['sha256']} manifest={manifest}")
            continue
